...
NESS <- function(GCP = NULL, data, cell_type = NULL, rareness = FALSE, data.name,
    ...
    prev_stability <- NULL #avoid first round error
    ...
    for (j in seq_along(GCP)) {
        ...
        perp <- GCP[j]
        set.seed(1000000000 + j)
        knn.mat <- list()
        knn.info <- NULL

        if (method == "tsne") {
            for (i in seq_len(N)) {
                set.seed(1000000000 + i)
                if (i == 1) {
        ...
        # rare.mean.vec helps to find the median rareness
